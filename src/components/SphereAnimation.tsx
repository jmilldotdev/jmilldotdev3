...
>(({ className = "", onShatter }, ref) => {
  ...
  const createWireframeFragments = () => {
    ...
    const shakeEffect = () => {
      ...
      const progress = Math.min(elapsed / 400, 1); // 400ms shake duration
      ...
      if (progress < 1) {
        ...
        sphereGroupRef.current.position.x = Math.sin(elapsed * shakeFreq * 0.01) * shake;
        sphereGroupRef.current.position.y = Math.cos(elapsed * shakeFreq * 0.013) * shake;
        sphereGroupRef.current.position.z = Math.sin(elapsed * shakeFreq * 0.007) * shake;
    ...
    function animate() {
      ...
      if (!isShatteredRef.current && sphereGroupRef.current) {
        ...
        if (Math.random() > 0.98) {
          ...
          if (redSphereRef.current) {
            const redMaterial = redSphereRef.current.material as THREE.LineBasicMaterial;
