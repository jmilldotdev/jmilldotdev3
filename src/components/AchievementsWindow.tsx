...
export const AchievementsWindow: React.FC<AchievementsWindowProps> = (props) => {
  ...
  return (
    <BaseWindow {...props} data-window="true">
      <div className="p-4 h-full flex flex-col">
        {/* Header Stats */}
        <div className="mb-4">
          <h2 className="text-[#00FFFF] font-mono text-lg mb-2">Achievement Progress</h2>
                ...
                row.map((achievement, x) => {
                  ...
                  return (
                    <div
                      ...
                      `}
                      onMouseEnter={() => achievement && setHoveredAchievement(achievement)}
            ...
            {hoveredAchievement ? (
              <div>
                <div className="flex items-center gap-3 mb-4">
                  <div className={`w-12 h-12 border border-[#00FFFF] flex items-center justify-center ${
                    hoveredAchievement.unlocked ? 'bg-[#00FFFF]/20' : 'bg-black/50'
                  }`}>
                  ...
                  <div className={`px-2 py-1 border font-mono text-xs ${
                    hoveredAchievement.unlocked 
                ...
                {hoveredAchievement.unlocked && hoveredAchievement.unlockedAt && (
                  <div className="text-[#00FFFF]/60 font-mono text-xs">
                    <div>Unlocked:</div>
                    <div>{hoveredAchievement.unlockedAt.toLocaleDateString()}</div>
                    <div>{hoveredAchievement.unlockedAt.toLocaleTimeString()}</div>
                  </div>
                )}

                <div className="mt-4 text-[#00FFFF]/40 font-mono text-xs">
                  Position: ({hoveredAchievement.x}, {hoveredAchievement.y})
                </div>
              </div>
            ) : (
